...
# HO-FN-016 Compiled Surface Call-Site And Index Dispatch Ack
...
1. OxFunc owns function/operator semantics, catalog-index dispatch, resolved call-site handles, scratch buffers, and function metadata.
...
3. `SurfaceCallSite`, `SurfaceCallRuntime`, and `SurfaceCallScratch` are the right seam for generic compiled formula evaluation without moving function-specific semantics into OxFml.
4. `SurfaceCallSite` metadata is the preferred source for future purity, volatility, host-interaction, callable-argument, and hoistability gates.
...
1. compiled evaluation plans retain resolved `SurfaceCallSite` handles for ordinary calls, operators, special operator lanes, reference operators, implicit intersection, and built-in callable slots;
2. evaluator runtime setup flows through `SurfaceCallRuntime`;
3. built-in callable `invoke_many` uses reusable `SurfaceCallScratch` for repeated call-site invocation;
...
2. generic compiled expression-node lowering beyond the current resolved-call-site floor,
3. reusable trace templates for prepared-call trace mode,
4. metadata-driven hoisting under explicit runtime-context policy,
