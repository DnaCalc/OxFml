...
- open_lanes:
  ...
  - the `LET` / `LAMBDA` seam pin-down agenda is now explicit, and typed invocation over opaque callable identity is now locally exercised; OxFunc's latest note narrows the minimum shared callable carrier candidate to opaque callable identity plus `origin_kind`, `capture_mode`, `arity_shape`, and `invocation_contract_ref`, with parameter/capture/body detail remaining provenance for the current phase
  - OxFml now mirrors the current-phase shared packet and carrier set explicitly in `docs/spec/formula-language/OXFML_OXFUNC_SHARED_INTERFACE_FREEZE_CANDIDATE_V1.md` rather than leaving the freeze candidate only in note traffic
  ...
  - the smaller callable carrier plus the current `W041` / `W042` / `W043` / `W052` packet floor is now mirrored in `docs/spec/formula-language/OXFML_OXFUNC_SHARED_INTERFACE_FREEZE_CANDIDATE_V1.md` and read locally as aligned with OxFunc's freeze-promotion handoff rather than remaining only note-level convergence
  ...
  - the remaining `W032` pressure is broader local consumer/model execution and any later concrete mismatch, not another broad OxFml/OxFunc interface-shape redesign
  - the newest OxFunc note now also asks for a real OxFml-backed preparation adapter and pinned seam-fixture corpus, so the next concrete artifact-driven closure lanes are `W049` and `W050`
  - the typed context/query bundle, returned publication-aware value surface, and runtime provider-consumer model for the already-covered application scope now have first local packet types, deterministic tests, and partial propagation through evaluation/host/candidate/commit paths; `RTD`, `INFO` unsupported-query, and `CELL` provider-failure now give live typed host/provider return lanes, but the packets are not yet canonically frozen on the OxFml side
