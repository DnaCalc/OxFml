...
- open_lanes:
  - OxFml now has a first real local consumer/model packet:
    ...
    and the single-formula host plus managed session path now consume the snapshot-ref packet during broader execution flow, but the runtime interface is not yet fully propagated through every compile/session/service surface
  - OxFml now prefers a cleaner runtime-only shape plus explicit CSV/export mapping, and that preference is backed by the first local provider model plus host consumption plus explicit session-surface snapshot-ref carriage, but not yet by broader multi-surface consumer use
  - deterministic local evidence now exists for snapshot pinning and surface lookup through the first provider model, and the managed session path now carries `LibraryContextSnapshotRef` explicitly on prepared/opened sessions, but broader generation/update behavior remains unexercised
  ...
  - deterministic local evidence for the broader session-path snapshot-ref carriage now exists in `crates/oxfml_core/tests/session_service_tests.rs`
