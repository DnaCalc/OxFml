...
- **Remaining gaps**: the authoritative published first-wave table is now confirmed by OxFunc to be 45 scenario ids and the local machine-readable corpus now admits all 45 of those ids; mismatch reporting against downstream packet artifacts is still narrow; worksheet `CALL` / `REGISTER.ID` remains outside the first adapter floor and now has a separate bounded owner; `W053` is now treated by OxFunc as sufficient for the current grouped-aggregation regression floor, but the surrounding shared seam packets still remain only partially frozen.
- **Why still open**: `W049` now has a real local adapter artifact and `W050` now covers the whole current pinned scenario table with explicit OxFml-side publication and bind-reject rules for the former `C12`/`C14` residuals; `W053` now satisfies its bounded local adapter-evidence scope with grouped-aggregation, helper-rejection, `HYPERLINK`, and `IMAGE` coverage and OxFunc now treats it as the current regression floor, but broader packet-diff scaling and the remaining `W052` field/ownership freeze still remain open.
...
- **Remaining gaps**: OxFunc and OxCalc now both align at note level on the direct-packet field names, seven-field descriptor, current-phase funnel-family ownership, and snapshot-generation split; promotion to shared seam-freeze text is still pending, and broader snapshot-acknowledgment/publication consequences remain outside the current first packet.
- **Why still open**: the local packet and evidence now exist, and the packet read is converged at note level on both sides, but the lane remains open until that converged packet is promoted beyond note-level convergence.
