...
- **Canonical owner**: active owner `W048`; diagnostic precision follow-through owner `W067`.
...
- **Status**: validated
- **Current floor**: OxFml editor function help and deterministic function-completion proposals now consume OxFunc registry truth through `oxfunc_core::registry`: built-in help/proposals use `builtin_registry()`, `EditorEnvironment` can receive a host/UDF-mutated `FunctionRegistry`, UDF registry entries surface in help and proposals, capability tweaks can be represented through `CapabilityOverlay`, capability-denied functions are filtered from proposals while explicit function help can still expose registry signature metadata with availability state, unknown registry entries return no `FunctionHelpPacket`, and `LibraryContextSnapshot` is preserved as availability/admission/provenance overlay data rather than a comprehensive function list. Current OxFunc `RegistryFunctionMeta` is consumed through `FunctionEntry.meta`, including owned runtime `function_id` and arity. `LibraryContextSnapshotEntry.arity_shape_note` and the old string-arity parser/synthetic `argN` help path have been removed from ordinary source and fixtures.
- **Remaining gaps**: DNA OneCalc still owns downstream removal of its host-owned comprehensive function-list and temporary snapshot-bridge wiring after consuming this OxFml revision. HO-FN-014 registry snapshot identity and UDF/name-resolution invalidation design remains a separate follow-up.
...
- **Status**: in-progress
- **Current floor**: OxFunc has filed `HO-FN-014` to separate canonical function-registry mutation from OxFml formula binding, name resolution, editor help, and host cache invalidation. OxCalc `HANDOFF-CALC-005` now adds the W051 host-context pressure case: bare call and non-call resolution must be settled against Excel-observed built-in, registered-UDF, workbook/sheet defined-name, and defined-name `LAMBDA` behavior before TreeCalc host names are mapped onto that lane. The current W074 planning/spec tranche spells the required oracle-matrix row shape, keeps TreeCalc names/lambda-valued nodes mapped only to the closest Excel defined-name lanes, defines the generic host hook and host-reference bind-result fields, names prepared identity/cache invalidation inputs across registry, structure, host namespace, caller context, table context, and resolution-rule changes, preserves structured-reference binding on the existing table-context packet, and adds the DNA OneCalc no-host-reference LET/LAMBDA lexical guardrail. A first runtime/replay pass-through now exposes product-neutral `RuntimeHostFormulaContext` and `RuntimeHostReferenceBindResult` facts, carries them through prepared identity and replay projection, and keeps sample host-reference labels generic rather than TreeCalc-shaped. A no-host-namespace runtime/replay guardrail now proves a returned `LAMBDA` with lexical capture executes without emitting host formula context or host-reference bind-result facts. The `W074_CALC005_NAME_CALL_PRECEDENCE_ORACLE_MATRIX.csv` intake artifact now has partial Excel COM 16.0 observations for selected built-in/defined-name, UDF/defined-name, sheet/workbook defined-name, defined-name `LAMBDA`, lexical-local, late-UDF-registration, and UDF-removal rows. OxFml currently supports registry-backed editor help/completion and UDF registry views, but formula-call bind/evaluation still has static built-in lookup lanes outside the W068 editor metadata cleanup.
- **Remaining gaps**: registry snapshot identity in bind/editor cache keys, formula-call binding against registry views for UDF-aware contexts, `#NAME?` recovery after late UDF registration, unregister/capability-denial invalidation beyond the observed unregister row, full Excel-oracle-backed UDF-vs-defined-name and defined-name-LAMBDA precedence coverage, explicit host-reference bypass evidence, host namespace mutation invalidation evidence, structured-reference table-context mutation evidence, and registered-external reconciliation.
...
- **Status**: validated
- **Current floor**: OxFunc `HO-FN-016` / `W096` has been reviewed as the target surface-call shape. OxFml now carries resolved `SurfaceCallSite` handles in compiled formula evaluation paths, keeps trace mode explicitly context-owned and default-off, uses reusable `SurfaceCallScratch` storage for built-in callable `invoke_many` batching, coalesces callable stack guards across helper batches, carries an ignored W075 manual release-mode timing fixture for the helper-loop and Mandelbrot controls, routes ordinary formula evaluation through an `EvaluationFrame` owner for trace/callable-registry/root-helper/scratch/recursion/stack-guard state, caches callable-argument ordinals on compiled call sites, decodes prepared helper-batch arguments once while assigning local callable slots and calculating recursion cost, and has a persistent layered `HelperBindingFrame` boundary that preserves case-insensitive helper semantics, avoids cloned helper frames for LET child scopes, and prepares the evaluator for true lexical slot vectors. A first runtime slot-overlay experiment preserved evaluator semantics but regressed the Mandelbrot fixture, so it is documented in `W075` as not promoted. The current promoted W075 floor now includes frame-owned helper slot arrays, direct helper-local slot reads where a compiled slot is known, narrowed captured-slot closure construction, slot-frame lambda parameter updates in helper batches and direct invocation, shared ownership for stored lambda bodies and parameter arrays, parsed numeric/string literal payloads, explicit compiled `LET` / `LAMBDA` / `IF` / `IFERROR` nodes, explicit ordinary `SurfaceCall` nodes for resolved OxFunc calls, pooled frame-owned `SurfaceCallScratch` with a scratch stack for nested ordinary compiled calls, value-only ordinary-call trace allocation discipline, strict context-free precomputed subtrees whose OxFunc call-site metadata says they are pure, and conservative slot-only execution for lambda-free slotted `LET` blocks.
- **Remaining gaps**: none within W075 closure scope. Future optimizer follow-ons are profiling-driven node-indexed buffers, broader captured-helper/reference-sensitive hoisting, optional static trace-template extraction if trace-on overhead becomes material, and wider workbook-level graph planning.
...
- **Status**: validated
- **Current floor**: OxFml has processed the DNA OneCalc conditional-formatting predicate handoff in `W070`: publication now evaluates `blanks`, `noBlanks`, `errors`, `noErrors`, and relative `dates` predicates, and `SingleFormulaHost` passes `TypedContextQueryBundle.now_serial` into publication so date predicates use the runtime clock seed. The non-locale custom-format follow-up covers selected section colour-token publication through `VerificationPublicationSurface.effective_font_color`, condition/colour header ordering, and fourth-section text `@` rendering with verbatim fallback when no fourth section exists. OxFml consumes OxFunc W094 profile facts for locale-keyed month/weekday rendering, profile-aware General decimal rendering, short-date parsing, currency layout, invariant custom-format numeric tokens, and `[$-LCID]` locale-prefix rendering.
- **Remaining gaps**: none within the bounded W070 handoff-processing scope. Full Excel custom-format and locale parity remain broader W057-family work, not W070 residue.
- **Why still open**: not applicable for W070; downstream DNA OneCalc cleanup and broader formatting parity require separate owner lanes if taken up later.
...
- **Status**: in_progress
- **Current floor**: W073 first slice adds `VerificationConditionalFormattingRule.typed_rule` metadata for color-scale stops, data-bar options, icon-set thresholds, rank options, and average-rule flags. The evaluator now requires typed metadata for `colorScale`, `dataBar`, `iconSet`, `top`, `bottom`, `aboveAverage`, and `belowAverage` options; the W072 bounded `thresholds` convention is intentionally ignored for those families.
