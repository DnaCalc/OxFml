...
- **Exact unblock steps**: add deterministic evidence for the remaining `W074_CALC005_NAME_CALL_PRECEDENCE_ORACLE_MATRIX.csv` gaps: bare table-name collision behavior if it is admitted into W074, stable table row membership/order and exact header/totals packet facts, host namespace mutation invalidation beyond explicit host-reference pass-through, and any missing Excel black-box rows needed for broader workbook/sheet/UDF/defined-name combinations; only then promote the name/call rule beyond provisional.
- **Recommendation**: wait
- **Opened**: 2026-05-22
