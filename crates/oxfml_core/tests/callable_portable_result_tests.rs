...
fn top_level_lambda_with_capture_surfaces_portable_callable_payload() {
    ...
    host.set_defined_name_value("Cap", CalcValue::number(10.0));
    ...
    assert_eq!(
        captured.binding,
        Some(DefinedNameBinding::Value(CalcValue::number(10.0)))
    );
...
fn ordinary_top_level_result_has_no_portable_callable() {
    ...
    let output = host
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("recalc should succeed");
    assert_eq!(output.published_worksheet_value, CalcValue::number(3.0));
...
fn portable_callable_round_trips_through_defined_name_supply_and_invokes() {
    ...
    producer.set_defined_name_value("Cap", CalcValue::number(10.0));
    ...
    consumer.set_defined_name_value("Cap", CalcValue::number(10.0));
    let output = consumer
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("consumer recalc should succeed");

    // `Cap=10` resolves live in the consumer, so MIN(99,10)=10.
    assert_eq!(output.published_worksheet_value, CalcValue::number(10.0));
}
...
fn portable_callable_round_trip_resolves_capture_live_in_consuming_scope() {
    ...
    producer.set_defined_name_value("Cap", CalcValue::number(5.0));
    let produced = producer
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("producer recalc should succeed")
        .portable_callable
    ...
    consumer.set_defined_name_value("Cap", CalcValue::number(100.0));
    let output = consumer
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("consumer recalc should succeed");
    assert_eq!(
...
fn let_bodied_top_level_lambda_captures_outer_defined_name() {
    ...
    host.set_defined_name_value("Cap", CalcValue::number(7.0));
    let output = host
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("recalc should succeed");
...
fn nested_lambda_shadows_outer_param_and_captures_outer_defined_name() {
    ...
    host.set_defined_name_value("Cap", CalcValue::number(3.0));
    let output = host
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("recalc should succeed");
...
fn array_literal_bodied_lambda_with_capture_surfaces_payload() {
    ...
    host.set_defined_name_value("Cap", CalcValue::number(9.0));
    let output = host
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("recalc should succeed");
...
fn produce_capped_callable(cap: f64) -> oxfml_core::eval::CallableDefinedNameBinding {
    ...
    producer.set_defined_name_value("Cap", CalcValue::number(cap));
    let produced = producer
        .recalc(None, Some(&oxfml_en_us_locale_context()))
        .expect("producer recalc should succeed")
        .portable_callable
