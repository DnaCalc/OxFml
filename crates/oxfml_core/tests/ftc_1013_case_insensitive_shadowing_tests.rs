...
fn array_shadow_expected() -> CalcValue {
    CalcValue::array(
        CalcArray::from_rows(vec![
            vec![CalcValue::error(WorksheetErrorCode::Div0)],
            vec![CalcValue::number(1.0)],
            vec![CalcValue::number(0.5)],
            vec![CalcValue::number(1.0 / 3.0)],
        ])
        .expect("column array"),
    )
}
...
fn evaluator_respects_case_insensitive_lambda_parameter_shadowing_simple_invocation() {
    ...
    assert_eq!(hit.oxfunc_value, CalcValue::number(0.5));
