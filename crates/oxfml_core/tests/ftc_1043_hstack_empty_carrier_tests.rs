...
use oxfml_core::eval::{evaluate_formula, EvaluationContext};
...
fn row_numbers(values: &[f64]) -> EvalValue {
    EvalValue::Array(
...
fn evaluator_collapses_hstack_empty_carrier_formula_ftc_1043() {
    ...
    assert_eq!(output.oxfunc_value, EvalValue::Error(WorksheetErrorCode::Calc));
}
...
fn runtime_collapses_hstack_empty_carrier_formula_ftc_1043() {
    ...
    assert_eq!(result.published_worksheet_value, EvalValue::Error(WorksheetErrorCode::Calc));
    assert_eq!(result.verification_publication_surface.visible_value_text, "#CALC!");
}
...
fn hstack_collapse_applies_to_direct_take_zero_carriers() {
    ...
    for (index, formula) in collapse_cases.iter().enumerate() {
        ...
        assert_eq!(runtime.published_worksheet_value, EvalValue::Error(WorksheetErrorCode::Calc));
...
fn hstack_without_empty_carrier_remains_array_control() {
    ...
    assert_eq!(runtime.published_worksheet_value, row_numbers(&[3.0, 8.0, 1.0, 9.0, 2.0]));
    assert_eq!(runtime.verification_publication_surface.visible_value_text, "3");
}
