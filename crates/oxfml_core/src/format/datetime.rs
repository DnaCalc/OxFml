...
pub fn looks_like_date_format(section: &str) -> bool {
    ...
    has_date_tokens && !contains_unsupported_time_tokens(&lower) && !lower.contains('#') && !lower.contains('0')
}
...
fn weekday_from_ymd(year: i64, month: i64, day: i64) -> usize {
    ...
    let h = (day + (13 * (month + 1)).div_euclid(5) + k + k.div_euclid(4) + j.div_euclid(4)
        + 5 * j)
        .rem_euclid(7);
    match h {
        0 => 6,
