    ...
    fn visit_reference_expr(&mut self, reference: &ReferenceExpr) {
        match reference {
            ReferenceExpr::Atom(NormalizedReference::Name(name))
                if matches!(name.kind, crate::binding::NameKind::MixedOrDeferred) =>
            {
                self.push_evaluation_requirement(EvaluationRequirement::NameCarrierDeferred);
                self.push_capability_requirement("name_formula_carrier");
            }
...
pub fn lookup_function_meta(function_name: &str) -> Option<FunctionMeta> {
    match function_name {
        ...
        "ISOMITTED" => Some(ISOMITTED_META),
        "MATCH" => Some(MATCH_META),
        "MAKEARRAY" => Some(MAKEARRAY_META),
        "MAP" => Some(MAP_META),
        "N" => Some(N_META),
        "NOW" => Some(NOW_META),
